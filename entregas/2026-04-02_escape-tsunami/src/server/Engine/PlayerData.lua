...
function PlayerData.get(p: Player): any      return _data[p] end

function PlayerData.sync(p: Player)
	if _data[p] then _sync:FireClient(p, _data[p]) end
end

function PlayerData.addMoney(p: Player, n: number)
	local d = _data[p]; if not d then return end
	d.money += n; PlayerData.sync(p)
end
...
function PlayerData.addBrainrot(p: Player, id: string, qty: number?)
	...
	for _, e in d.brainrots do
		if e.id == id then e.qty += n; PlayerData.sync(p); return end
	end
...
function PlayerData.removeBrainrot(p: Player, id: string): boolean
	...
	for i, e in d.brainrots do
		if e.id == id then
			e.qty -= 1
			if e.qty <= 0 then table.remove(d.brainrots, i) end
			PlayerData.sync(p); return true
		end
	end
	return false
end
...
function PlayerData.countBrainrots(p: Player): number
	local d = _data[p]; if not d then return 0 end
	local t = 0; for _, e in d.brainrots do t += e.qty end; return t
end

function PlayerData.unlockJump(p: Player, id: string)
	local d = _data[p]; if not d then return end
	for _, j in d.unlockedJumps do if j == id then return end end
...
function PlayerData.hasJump(p: Player, id: string): boolean
	local d = _data[p]; if not d then return false end
	for _, j in d.unlockedJumps do if j == id then return true end end
	return false
end
