...
local function findMapZBounds(): (number, number, number, number)
	...
	for _, obj in ws:GetDescendants() do
		...
		if waterTags[bp.Name] then continue end
	...
	if minZ == math.huge then minZ = -200; maxZ = 200; minX = -40; maxX = 40 end
...
local function startWave(speedOverride: number?, killer: Player?)
	if waveActive then return end
	waveActive = true
	waveCount += 1

	local cfg   = _cfg.WAVE
	local speed = speedOverride
		or math.min(cfg.SPEED + math.floor(waveCount / 5) * 4, cfg.SPEED_MAX)
	...
	print(string.format(
		...
		waveCount, startCF.Position.Z, endZ, speed, mapWidth
	))
	...
	for _, p in Players:GetPlayers() do survived[p] = true end
	...
	local conn = wave.Touched:Connect(function(hit)
		...
		if hrp and isInSafeZone(hrp.Position) then return end
		local d = PD.get(pl)
		if d and d.hasShield then return end
		survived[pl] = false
		local h = char:FindFirstChildOfClass("Humanoid")
		if h then h.Health = 0 end
		if killer then
			local kd = PD.get(killer)
			if kd then kd.noobsKilled += 1; PD.sync(killer) end
		end
	end)
	...
	task.spawn(function()
		...
		for pl, ok in survived do
			if ok and pl.Parent then
				...
				local d = PD.get(pl)
				if d then d.wavesSurvived += 1; PD.sync(pl) end
			end
		end
...
function WaveSystem.init(cfg: any)
	...
	useToken.OnServerEvent:Connect(function(p, speedStr: string)
		local PD   = require(script.Parent.PlayerData)
		local d    = PD.get(p); if not d then return end
		local spd  = tonumber(speedStr) or 40
		local cost = if spd >= 100 then 10 elseif spd >= 70 then 5 else 1
		if d.waveTokens < cost then return end
		d.waveTokens -= cost; PD.sync(p)
		startWave(spd, p)
	end)
